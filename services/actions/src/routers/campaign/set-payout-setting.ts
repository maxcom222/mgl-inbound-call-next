...
export const setCampaignPayoutSettingsHandler = async (
  req: Request,
  res: Response
): Promise<void> => {
  console.log('req.body', req.body);

  const { campaignId, payoutSettings } = req.body;

  try {
    const GET_DEFAULT_PAYOUT_SETTINGS_ID_QUERY = gql`
      query getDefaultPayoutSettingsId($campaignId: Int!) {
        contactcenter_default_payout_settings(
    ...
    const defaultPayoutSettingsIdResponse = await requestGQL({
      query: GET_DEFAULT_PAYOUT_SETTINGS_ID_QUERY,
      isAdmin: true,
      variables: {
        campaignId
      }
    ...
    const defaultPayoutSettingsIdResult = defaultPayoutSettingsIdResponse.contactcenter_default_payout_settings;
    console.log('defaultPayoutSettingsIdResult', defaultPayoutSettingsIdResult);

    for (const defaultPyaoutSetting of defaultPayoutSettingsIdResult) {
      ...
      const getDefaultPayoutSettingCriteriaResponse = await requestGQL({
        ...
        isAdmin: true,
        variables: {
          dePayoutSettingId: defaultPyaoutSetting.id
        }
      });
    ...
    for (const defaultPayoutSetting of payoutSettings) {
      defaultPayoutSetting.campaignId = campaignId;
    ...
    res.status(201).json("successfully set payout Settings");
  } catch(error) {
    console.log(error);
...
export default setCampaignPayoutSettingsHandler;