...
export const setWeightHandler = async (
  req: Request,
  res: Response
): Promise<void> => {
  console.log('req.body', req.body);

  const { campaignId, callRouteId, weight } = req.body;
  ...
  console.log('setWeightHandler - appConfig', appConfig);
...
//   const { workspaceSid } = twilioConfig?.data;
  ...
  try {
    ...
    const UPDATE_PRIORITY_QUERY = gql`
      mutation update_routing_priorities($id: Int!, $weightValue: Int!) {
...
export default setWeightHandler;
